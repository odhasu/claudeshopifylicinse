...
export const SITE_URL = process.env.SITE_URL || 'https://claudecodethemeshopify.vercel.app';
...
interface ResendInstance {
  emails: {
    send(params: { from: string; to: string; subject: string; html: string }): Promise<{ data?: { id?: string } }>;
  };
}

function getResend(): ResendInstance | null {
  if (!RESEND_API_KEY) return null;
  const { Resend } = require('resend') as { Resend: new (key: string) => ResendInstance };
  return new Resend(RESEND_API_KEY);
}

function getMailerOrSkip(email: string | null | undefined, skipMessage: string): ResendInstance | null {
...
async function sendEmailWithResend({ resend, to, subject, html, successLog, errorLog }: SendEmailParams): Promise<void> {
  try {
    ...
    console.error(errorLog, (err as Error).message);
  }
}
...
export async function sendWelcomeEmail({ email, name, licenseKey, plan }: WelcomeEmailParams): Promise<void> {
...
export async function sendTicketReplyEmail({ email, name, message, ticketId }: TicketReplyEmailParams): Promise<void> {
