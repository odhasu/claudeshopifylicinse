...
export default function chat(settings: Record<string, unknown>, _products: unknown[] | null, colors: Record<string, string>): string {
  const name = (settings.name as string) || 'Support';
  const greeting = (settings.greeting as string) || 'Hey! How can I help?';
