...
export default function testimonials(settings: Record<string, unknown>): string {
  const images = (settings.images as TestimonialImage[]) || [];
  if (!images.length) return '';
  const headline = (settings.headline as string) || 'What Our Customers Say';
  const speed = (settings.animationSpeed as number) || 20;
  const imgHeight = (settings.imageHeight as number) || 250;
