...
export default function headerMinimal(settings: Record<string, unknown>, _products: unknown[] | null, _colors: Record<string, string>, cfg: RendererConfig): string {
  const logo = (settings.logo as string) || cfg.logoUrl || '';
  const links: { text: string; url: string }[] = [];
  for (let i = 1; i <= 6; i++) {
    const text = settings['nav_link_' + i + '_text'] as string | undefined;
    const url = settings['nav_link_' + i + '_url'] as string | undefined;
